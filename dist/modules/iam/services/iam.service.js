...
const iam_repository_1 = require("../repositories/iam.repository");
const avatar_storage_util_1 = require("./avatar-storage.util");
const ACCESS_TOKEN_TTL_MS = 15 * 60 * 1000;
const REFRESH_TOKEN_TTL_MS = 7 * 24 * 60 * 60 * 1000;
class IamService {
    constructor() {
        this.iamRepository = new iam_repository_1.IamRepository();
    }
    hashText(value) {
        return (0, crypto_1.createHash)("sha256").update(value).digest("hex");
    }
    generateOpaqueToken(prefix) {
        return `${prefix}_${(0, crypto_1.randomUUID)()}_${(0, crypto_1.randomBytes)(24).toString("hex")}`;
    }
    buildLoginUserResponse(params) {
        return {
            id: params.user.id,
    ...
    async register(payload) {
        ...
        let resolvedAvatarUrl = payload.avatar_url;
        if (payload.avatar_file) {
            const savedAvatar = await (0, avatar_storage_util_1.saveUserAvatar)({
                userId: user.id,
    ...
    async login(payload, deviceInfo) {
        const user = await this.iamRepository.findUserByEmailOrPhone(payload.email_or_phone);
        if (!user) {
    ...
    async revokeSession(userId, sessionId) {
        const existingSession = await this.iamRepository.findSessionByIdAndUserId(sessionId, userId);
        if (!existingSession) {
