...
exports.LsmVideoRepository = void 0;
const postgres_1 = require("../../../shared/db/postgres");
class LsmVideoRepository {
    async findStudentTaskByTaskCode(studentId, classCode, taskCode) {
    ...
    async findClassWeekContentItemById(sourceId) {
        const result = await postgres_1.pool.query(`
      SELECT
        cwci.id,
        cwci.content_type,
    ...
    async findStudentClassWeekContentItemById(studentId, sourceId) {
        const result = await postgres_1.pool.query(`
      SELECT
        scwci.id,
        scwci.student_id,
    ...
    async findLectureVideoById(contentId) {
        const result = await postgres_1.pool.query(`
      SELECT
        lv.id,
        lv.title,
    ...
    async findVideoQuizEventsByLectureVideoId(contentId) {
        const result = await postgres_1.pool.query(`
      SELECT
        vqe.id,
    ...
    async findVideoQuizEventQuestionsByEventIds(eventIds) {
        if (!eventIds.length) {
            return [];
        }
        const result = await postgres_1.pool.query(`
    ...
    async findQuizQuestionsByIds(questionIds) {
    ...
    async findContentFileById(contentId) {
        const result = await postgres_1.pool.query(`
      SELECT
        cf.id,
        cf.title,
    ...
    async findQuizTestById(quizTestId) {
        const result = await postgres_1.pool.query(`
      SELECT
        qt.id,
        qt.title,
    ...
    async findQuizTestQuestionsByQuizTestId(quizTestId) {
        const result = await postgres_1.pool.query(`
      SELECT
        qtq.id,
        qtq.quiz_test_id,
...
exports.LsmVideoRepository = LsmVideoRepository;
