...
class CgeRepository {
    ...
    async findClassById(classId) {
      ...
      limit 1
    `;
        const result = await postgres_1.pool.query(query, [classId]);
    ...
    async createGroup(payload) {
        const inviteCode = "IXG-" + (0, crypto_1.randomUUID)().slice(0, 8).toUpperCase();
        const query = `
      insert into groups (
        name,
        description,
    ...
    async addGroupOwnerAsMember(payload) {
      ...
      returning *
    `;
        const result = await postgres_1.pool.query(query, [payload.group_id, payload.user_id]);
        return result.rows[0];
    }
    async findGroupById(groupId) {
      ...
      limit 1
    `;
        const result = await postgres_1.pool.query(query, [groupId]);
        return result.rows[0] || null;
    }
    ...
    async removeGroupMember(groupId, userId) {
      ...
      returning *
    `;
        const result = await postgres_1.pool.query(query, [groupId, userId]);
        return result.rows[0] || null;
    }
    async createJoinRequest(payload) {
      ...
      returning *
    `;
        const result = await postgres_1.pool.query(query, [payload.group_id, payload.requester_user_id]);
        return result.rows[0];
    }
    async findJoinRequestById(requestId) {
      ...
      limit 1
    `;
        const result = await postgres_1.pool.query(query, [requestId]);
        return result.rows[0] || null;
    }
    async approveJoinRequest(payload) {
      ...
      returning *
    `;
        const result = await postgres_1.pool.query(query, [payload.request_id, payload.reviewed_by]);
        return result.rows[0] || null;
    }
...
exports.CgeRepository = CgeRepository;
